...
Cette distance est d'autant plus grande que les rayons solaires suivent une trajectoire inclinée, c'est-à-dire que leur angle d'incidence est grand (figure~\ref{fig:angle-of-incidence}).
...
\subsection{Heure solaire, angle horaire}
...
Un paramètre appelé \emph{équation du temps} rend compte de ces irrégularités.

\footnote{\textit{``Remarque sur le mot ``équation'' : en astronomie ancienne, le terme ``équation'' désignait une correction ajoutée algébriquement à une valeur moyenne pour obtenir une valeur vraie.
